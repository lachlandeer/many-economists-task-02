...
Place the compressed ACS data extracted from IPUMS in the folder `src/data` and name it `usa_0001.csv.gz` (the default name from IPUMS).

To run the estimates and any necessary data cleaning enter the following in the command line followed by pressing return:

```
snakemake --cores NUM_CORES all --keep-going
```

Note that you must be in this project's working directory when running the snakemake command. For example,:

```bash
YOUR/FILE/PATH/manyeconomists$ snakemake --cores 12 all --keep-going
```
